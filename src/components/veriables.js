// Light Theme 1
export const light_button_color = '#4A154B';
export const light_bg_color = '#f0f0f0';
export const light_text_color = '#808080'; // Grey

// Light Theme 2
export const light2_button_color = '#3498db';
export const light2_bg_color = '#FBF0D9';
export const light2_text_color = '#5F4B32';

// Dark Theme 1
export const dark_button_color = '#2E8B57'; // Different button color
export const dark_bg_color = '#2d2d30';
export const dark_text_color = 'white'; // Grey

// Dark Theme 2
export const dark2_button_color = '#3498db';
export const dark2_bg_color = '#1E1E1E';
export const dark2_text_color = 'white';

// cell color

export const selected_rows_color = '#4d5d53';
export const selected_cell_color = '#C0C0C0';
export const selected_match_color = '#939393';

//storagekey
export const STORAGE_KEY = '@SudokuGameState';
export const STORAGE_KEY_STATS = '@SudokuGameStateGlobel';
export const STORAGE_KEY_Battels = '@SudokuGameStateBattels';
export const STORAGE_KEY_First_Play = '@SudokuGameStateFirstPlay';
export const STORAGE_KEY_Battels_Uncompleted =
  '@SudokuGameStateBattelsUncompleted';
