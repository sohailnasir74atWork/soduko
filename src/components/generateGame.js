...
export const sudokuGen = (level) => {
  let solvedSudoku = newGrid(CONSTANT.GRID_SIZE);
  let check = sudokuCreate(solvedSudoku);

  if (check) {
    // Create a deep copy of the solved Sudoku grid
    let originalSudoku = solvedSudoku.map(row => [...row]);

    // Remove cells to create the question
    let question = removeCells(originalSudoku, level);

    return {
      original: solvedSudoku,
      question: question
    };
  }
